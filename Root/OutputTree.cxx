...
void OutputTree::add_floats(const std::string& name, const std::vector<float>& vals) {
  if (floats_vars.count(name) == 0) {
    TBranch* b = Branch(name.c_str(), &floats_vars[name]);
